...
describe('Movie', () => {
    describe('getAll Service', () => {
        ...
        it("should successfully get a list of all movies on the SWAPI", async function () {
            ...
            for(let index = 0; index < result.results.length; index++){
                expect(result.results[index]).to.have.property("title")
                expect(result.results[index]).to.have.property("episode_id")
                expect(result.results[index]).to.have.property("opening_crawl")
                expect(result.results[index]).to.have.property("comment_count")
            }
        })
    ...
    describe('getSingleMovieComments Service', () => {
        before(async function () {
            movieComments = await addFakeCommentsToMovies()
        })
        it("should successfully post a comment on a movie", async function () {
            const newComment = await Movie.getSingleMovieComments({ movieId: 3})
            
            expect(newComment).to.be.an("array")
            expect(newComment[0].comment).to.be.equal("Some other random comment")            
        })
        after(async function () {
            for(let index = 0; index < movieComments.length; index++){
                await movieComments[index].destroy()
            }
        })
    })//end of addComment Service
})
